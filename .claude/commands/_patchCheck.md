...
⚠ **要有一台生產職（廚狐）在線** —— 「配方記錄版面」那一項只有學了配方的角色驗得到，
戰鬥職一律 `–`。2026-09-23 就是這樣漏掉：9/22 改版把配方記錄欄位整段搬 0x10（類別 1 格→5 格），
五台戰鬥職全綠，自動生產一到製作檯就「半成品做完了 0 個」。這組是 disp8 偏移，AOB 跟不了。
...
2026-09-22 大改版：視窗類 **+0x10**（清單控制項項目向量 0x150→0x160、輸入框字串 0xF8→0x108、
對話「最後一頁」0x148→0x158、GETWIDGET 內 0xC8→0xD8）、管理器與狀態物件 **+8**
（OFF_TARGET 0x270→0x278、TABLE_OFF、MEMBERS_OFF、ROBOT_READY、mall 兩格、`energy.OFF_ENERGY`
0x34→**0x3C**）。⚠ 那次 `entity.VT_STATE` **有中**晶化照樣搬 —— 「VT_STATE 沒中才連動」
的舊規則作廢，**晶化那組改版後一律重驗**。另外 4 段沒中都是 pattern 裡寫死了結構偏移／
vtable 槽（`_auto_mask` 不遮模組外的小數字）→ **那類一律手寫 `??`**。見 memory `patch-2026-09-22`。
...
* **`energy.OFF_ENERGY`（＋它 +4/+8/+12 那三欄）** —— 晶化介面整個是 Lua 寫的，
  ...
  ⚠ 改版後**一律重驗**（9/22 起）：`entity.VT_STATE` 沒中＝必搬；有中≠不搬
  （9/22 VT_STATE 有中、晶化照樣 0x34→0x3C）。
...
* **對不上** → 先看下面那招能不能自己補；真的補不了才請使用者重新解包
  （`D:\RPGViewer` 是 GUI，我們代跑不了），然後把**下面清單裡的 build 工具全部重跑一輪**再蓋章。

### ★★★ 資料表清單：改版後一律重新生成（有表對表，GAMEDATA 最權威）

> 使用者 2026-09-06：「有表對表，GAMEDATA 這是最權威不會錯」「如果你有要產生什麼對表用的
> 東西，記得要在 _patchCheck 說明有更新記得要重新生成表」。
> ⛔ **新增任何一張表／build 工具，一定要同時加進這張清單**（CLAUDE.md 第 0 條）。

全部吃**解包後的 GAMEDATA**（`GAMEDATA/setting`、`GAMEDATA/shape`），沒參數就是專案根目錄那份：
...
| `py tools\build_furniture.py` | `assets/furniture.tsv.gz` | 傢俱基礎魔力值（furniture.xml 擺設分數，經 item.xml 動態資料1 對到）＋傢俱魔力錘擲出範圍（動態資料1／2）→ 傢俱強化分頁；過期＝新傢俱／新錘子不列（安全退化），`--check` 只比對 |
...
| `py tools\build_house_zones.py` | `assets/house_zones.json` | **可展示房屋的格子**（`.mpc` 地形格 byte[2] bit 0x20，5 主城＋天使住宅區；`app/game/housetrip.py` 自動擺房子挑格用）→ 過期＝新城／改過的區域挑不到格或挑到不能擺的格（伺服器會拒絕、工具換格重試，擺不上 2 分鐘就退回原本製作檯，不會送錯東西）；改版後重跑（`--check` 只比對） |
...
2. ☐ **④ 層有收尾**：蓋了章，或丟出一件講清楚「哪張表、差幾列、影響什麼」
   的明確待辦。⛔ 不准把亮著的警示留給使用者。
3. ☐ **沒有 AOB 的偏移確認過**（尤其 `energy.OFF_*` —— 一律重驗，別看 `VT_STATE`）。
