...
#   還是卡**，所以拿掉了。那個 A/B 只有兩輪、而且是在我自己的測試迴圈裡跑的
#   （沒有掃描、換目標、冷卻那些），不足以證明它有用。
#   函式版面已解出來，要再試隨時可以加回去：
#       0x559FBE(目標實體ID, u16)  cdecl，兩個參數都只寫進封包緩衝，
#       沒有指標解參考，呼叫安全。
...
def strike(mover, pf_this: int, skill_id: int, target_id: int,
    ...
    tile_x/tile_y: 施放封包的第 3、4 個參數。
        ★★ **打怪時要給 0**，跟遊戲一樣 —— 攔包實證（雪狐、黑狐各一份）
          遊戲自己送的是 `0x664627(技能ID, 目標ID, 0, 0, 0)`。
          掛機那邊現在固定傳 (0, 0)。
        ⚠ 只有**順移那類對地技能**才需要真的座標（靠它指定落點，
          見 [[teleport-skill]]）；把順移放在攻擊鍵時要自己傳進來。
