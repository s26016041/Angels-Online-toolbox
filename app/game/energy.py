...
# ⚠⚠⚠ **改版整組搬家過兩次：0xB8 → 0x54（2026-08-11，−0x64）
#      → 0x34（2026-08-25，−0x20）。這是全專案唯一沒有 AOB 自動跟上的偏移。**
...
#
# 2026-08-25 又搬了一次（0x54 → 0x34），用同一招定的：
#   送 0x3F 後 diff 整個狀態物件 —— 白狐／北極狐 **+0x34 由 0 → 10**（能量），
#   黑狐 **+0x38 由 −1 → 8**（抽到的屬性那個哨兵值，最硬的錨）。
#   五台照新起點重讀全部合理（能量 0/0/10/0/10、抽到 −1 或 1/8、每次 0/10、
#   12 格點數嵐狐 13552/21340/3859/…）。
#   ⚠ 這次的搬家是「狀態物件開頭那幾個子物件被重排」的連帶結果：建構函式
#     從寫 vtable 到 +0x34/+0x38/+0x4C 改成 +0x18/+0x2C（見 [[patch-2026-08-25]]）。
#     所以**改版後只要 entity.VT_STATE 那段特徵沒中，就要順便重驗這一組**。
#   ⚠⚠ 舊偏移在 5 台裡有 **2 台照樣通過版面驗證**（讀到的是點數陣列後段，
#     剛好長得像合理值）→ 分頁安靜顯示錯的數字。逐欄驗證擋得住大部分、
#     但擋不住全部，不要以為有驗證就不會錯。
#
# 2026-09-22 大改版第三次搬（0x34 → 0x3C，+8；同一版 entity.OFF_TARGET、
#   quickbar.TABLE_OFF、team.MEMBERS_OFF 等一票偏移也都 +8），同一招定的：
#   送 0x3F 後 diff 狀態物件前 0x800 —— 5 台**+0x3C** 由 0 → 10/10/200（能量）、
#   黑狐 **+0x40 由 −1 → 8**、嵐狐 +0x40 −1 → 1（抽到的屬性哨兵）、+0x44 = 10
#   （每次點數）、+0x48 起 12 格點數（嵐狐 13520/21340/3859/1416/21600…，
#   跟 8/25 那次讀到的同一組數字量級一致）。verify_offsets 五台 ✘ → 改完 ✔。
OFF_ENERGY = 0x3C
...
# ⚠ 2026-08-25 從 1 億收到 1000 萬：舊偏移（搬家後的錯位置）讀到的垃圾值
#   8780 萬／4028 萬**通過了 1 億這道關**，害 5 台裡有 2 台安靜顯示錯的
#   數字、verify_offsets 也因此印成綠燈（它要每一台都失敗才算壞）。
#   真實累積點數的量級是萬（實測最大 21600），1000 萬已經有 500 倍餘裕。
#   ⚠ 這條驗的是「這裡還是不是那個欄位」——別再往下收到接近真實值，
#     那就變成業務值白名單了（per_roll 那次的教訓）。
