...
OFF_H = 0x28               # 高（格）
OFF_ROWS = 0x2C            # 列指標陣列
# ⚠ 出處＝檔頭那段反組譯（0x507F20）的 `imul ecx,edx,7`。
CELL = 7                   # 每格幾 bytes
# ⚠ 出處＝同段反組譯 `test byte ptr [eax+ecx+2],3` —— 旗標在每格第 2 個 byte。
...
OBJ_MASK = 1
MAP_MASK = 2               # bit1＝.mpc 載圖填的牆（2026-09-07 反組譯，見上；雷達 0x5858ea 畫成 0x7e0）
...
RETRY_GAP = 0.2
# ★ 終點落在牆上時往外找可走格的半徑。跟遊戲尋路對答案實測（見檔頭驗證段）：
#   9 個不一致全是「終點那格是牆」，遊戲自己也是回一條盡量靠近的路。
GOAL_RELAX = 4
