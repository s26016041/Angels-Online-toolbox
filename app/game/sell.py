...
# ★ 出處：反組譯 npcsalesellconfirm → 0x5D70AE 的 `push 0x28`（見檔頭），
#   線材長度並用使用者擷取那包（86 bytes）驗算過。
OPCODE = 0x28                # 賣東西給 NPC
# ⚠ 同一段反組譯的版面：內文 = 6 + 12×件數。
BODY_HEAD = 6                # 代號(u16) + 件數(u32)
# ⚠ 反組譯 0x6023AD：每件 12 bytes 直接抄物品 +0/+4 兩欄＋數量。
BODY_ENTRY = 12              # 序號 + 取得時間 + 數量
# ★ 出處：擷取實錄「確定賣出」前那包 talkaction 的動作碼就是 10（見檔頭）。
TALK_SELL = 10
