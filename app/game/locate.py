...
SIGS: tuple[Sig, ...] = (
    ...
    Sig("supply", "INTERACT_FN", "fn", None,
        ...
        0x0054A520),
    # ★★★「官方滑鼠點 NPC」真正動手的那一支 TryAct(eid, kind)（見 supply.py 檔頭）。
    #   thiscall，ecx = pathfinder_this()；kind 3＝跟 NPC 互動、4＝攻擊。
    #   在互動範圍內就當場開對話，不在範圍就用官方尋路走一步。
    #   錨：函式頭的堆疊版面 → `lea edi,[esi+0x35C]`（互動狀態欄，結構偏移＝很獨特）
    #   → `sub eax,3`（kind 3 分家的那道）。所有 rel8/rel32 跳躍全放萬用
    #   （函式體內的位移量，改版重編就會變，不能當錨）。已驗模組內唯一。
    Sig("supply", "TRY_ACT_FN", "fn", None,
        "55 8B EC 83 EC 28 53 8B 5D 0C 8B C3 56 8B F1 57 6A 01 59"
        " 8D BE 5C 03 00 00 83 E8 03 74 ?? 2B C1 0F 84 ?? ?? ?? ??"
        " 2B C1 74 ?? 89 0F E9 ?? ?? ?? ?? C7 07 02 00 00 00",
        0x00506784),
    ...
    Sig("login", "SRV_BEGIN", "off", 18,
        ...
        " 8B 91 ?? ?? 00 00 83 7C 32 ?? 00 74 3D",
        0x00000500),
    # ★ 2026-09-23「自動登入選錯伺服器」修法的三個全域（見 login.py 檔頭「四、」）。
    # 登入函式 LOGIN_FN 一開始就把選中那筆伺服器記錄的 ip/port 抄進兩個全域
    # （`0x538EAC`：`strncpy(LOGIN_IP, 記錄+0x40, 0xF)` / `mov [LOGIN_PORT],[記錄+0x50]`），
    # 之後 socket 就連這兩個 —— 所以「登入後讀它們跟目標記錄比」＝遊戲**真的**
    # 連去哪台，比讀清單選取狀態可靠。錨在那支函式整段骨架；兩個 call 的 rel32
    # 放萬用，三個全域位址由 _auto_mask 遮。
    Sig("login", "LOGIN_IP", "data", 14,
        "55 8B EC 56 8B 75 08 6A 0F 8D 46 40 50 68 14 27 8D 00 E8 ?? ?? ?? ??"
        " 8B 46 50 A3 24 27 8D 00 8D 46 60 6A 0F 50 68 28 27 8D 00 E8 ?? ?? ?? ?? 8B 46 70",
        0x008D2714),
    Sig("login", "LOGIN_PORT", "data", 27,
        "55 8B EC 56 8B 75 08 6A 0F 8D 46 40 50 68 14 27 8D 00 E8 ?? ?? ?? ??"
        " 8B 46 50 A3 24 27 8D 00 8D 46 60 6A 0F 50 68 28 27 8D 00 E8 ?? ?? ?? ?? 8B 46 70",
        0x008D2724),
    # 「上次選的伺服器索引」全域。登入畫面每次進場（vtable slot 1 = 0x5364F5）會
    # 清空重建清單，然後把這個索引選回去 —— 我們只改清單 +6 旗標的話，畫面一重建
    # 就被還原成上次那台。錨在那段「讀它 → 跟伺服器數比 → 選它／選 0 並歸零」；
    # 兩個 [ebp-xxx] 區域變數與 rel32 放萬用。
    Sig("login", "LAST_SERVER", "data", 1,
        "A1 A0 24 8D 00 8B CE 8B BD ?? ?? ?? ?? 3B 85 ?? ?? ?? ?? 7D 0D 33 DB 43 53 53 50"
        " E8 ?? ?? ?? ?? EB 16 33 DB 43 53 53 6A 00 E8 ?? ?? ?? ?? C7 05 A0 24 8D 00 00 00 00 00",
        0x008D24A0),
    ...
    Sig("gather", "VT_RESOURCE", "data", 0x17,
        ...
        0x007D87B4),
    # ── 副本傳點／機關：那支「每拍檢查有沒有人踩上來」的虛擬函式 ────
    # 它是「認得出哪些東西踩上去會送 0x0D」的唯一依據（見 app/game/portal.py）：
    # 物件 → vtable → +0x0C 的跳板 → 尾巴 jmp 的目標就是這一支。
    # 錨用它自己的骨架：讀旗標(+0x1FE)、節流欄(+0x204)倒數、`test eax,0x185`。
    # ⚠ 那三個結構偏移故意留在特徵裡當骨架 —— 真的一起搬家時這段會定位失敗
    #   （fn 清 0），portal.nearby() 就回 None 大聲停用，而不是拿別的類別的
    #   +0x1FE 當旗標亂報一堆假傳點。rel8/rel32 一律遮。
    # 建「踩到觸發物件」那一包（代號 0x0D、內文 11）並送出的那支：
    #   `0x5D94A4(玩家+0x1D0, 物件+0x1D0, u8 動作碼)`
    # 錨用 `push 11 / push 0x0D`（內文長度＋封包代號，這一組全模組只有它）
    # 加上三個欄位的寫入版面（+2 / +6 / +0x0A）。連線全域與兩個 rel32 遮掉。
    Sig("portal", "SEND_FN", "fn", None,
        "55 8B EC 83 EC 10 6A 0B 6A 0D 8D 4D F0 E8 ?? ?? ?? ??"
        " 8B 4D F4 8B 45 08 FF 75 FC 89 41 02 8B 45 0C 89 41 06"
        " 8A 45 10 88 41 0A FF 35 98 33 9F 00 E8 ?? ?? ?? ?? 59 59 C9 C3",
        0x005D94A4),
    Sig("portal", "TRIGGER_FN", "fn", None,
        "56 8B F1 0F B7 86 FE 01 00 00 66 85 C0 0F 84 ?? ?? ?? ??"
        " 8B 8E 04 02 00 00 85 C9 7E ?? 8D 41 FF 89 86 04 02 00 00 5E C3"
        " A9 85 01 00 00 74 ?? 8B 8E B0 00 00 00 56 E8 ?? ?? ?? ??"
        " 8B C8 85 C9 74 ??",
        0x00546A13),
