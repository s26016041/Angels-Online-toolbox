...
VT_SHIFT = 8              # 掃到的位址(VT_ENTITY) − 物件起點(VT_ENTITY2)；出處＝檔頭「兩個 vtable 相差 8 bytes」，反組譯 +0x1D0 ↔ OFF_ID +0x1C8
# 攻擊型動畫狀態（出手的那幾拍）。搭配距離就是「正在打我」的另一半訊號。
ATT_STATES = ("Att", "Att2", "Cast")
STATE_DEAD = "Dead"       # 屍體（最快也最準的死亡訊號）
STATE_RUNNING = "Run"     # 跑動中
# 「牠還在動」＝出手中或跑動中。⚠ 死活判斷**不用**它（見 looks_dead 的
# 「不准把血量歸零加回來」）—— 留著給呼叫端做別的判斷（例如零傷害繞打）。
...
def looks_dead(state: str, alive: bool = True, flag=None) -> bool:
    ...
    掛機那邊還有第三個訊號「**遊戲自己把目標欄清 0**」，那條天生只對當下鎖定
    的那一隻有效（目標欄只有一格），所以留在呼叫端，不放進這裡。

    ⛔⛔ **不准把「血量歸零」加回來**，不管加什麼附加條件都不行
      （2026-09-09 使用者連退兩次）：
      · 血量是 0~100 的**整數百分比** —— 王剩不到 1% 就讀成 0，
        「0 就是死」會讓還在打人的王被放掉（第一次退）。
...
#     （安全退化：不會誤判死）。
OFF_DEAD_FLAG = 0x3D6
DEAD_FLAG = 7             # 實測：+0x3D6 死亡當拍翻 0x07；活怪 7,800 次取樣出現 0 次（見上段）
...
class Entity:
    ...
    def hp_zero(self) -> bool:
        ...
        ⛔⛔ **這不是死亡訊號，不准拿來判死**（2026-09-09 使用者連退兩次，
          理由見 `looks_dead`）：那是 0~100 的整數百分比，王剩不到 1% 就是 0。
          留著只給「顯示」與診斷用。死活一律問 `looks_dead()`。
        """
        return self.hp == 0
