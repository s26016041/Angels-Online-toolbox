...
#     · 狀態物件 —— ★ 最省事：`[quickbar.MGR_PTR]` **就是狀態物件本人**，
#                   讀它的 +0 就是新的 VT_STATE（2026-08-25 就是這樣一次
#                   五台對帳確定的）。要交叉驗證的話，同一支建構函式裡也寫了
#                   player.VTABLE_RVA —— 從那段特徵的命中處往上找
#                   `mov [edi], imm` 就是這裡。
#                   ⛔ 不准拿別的 vtable 的位移量去推（2026-08-25 實測
#                     VT_STATE 位移 +0x2038、VT_PLAYER 才 +0x1FE8）。
VT_STATE = 0x007E5E10     # 玩家狀態物件（每個分身剛好 1 個）
VT_PLAYER = 0x007D8BF8    # 玩家自己的物件（每個分身剛好 1 個）

# --- 實體物件的欄位（基準 = 掃 VT_ENTITY 得到的位址）---
# ⚠ 出處：當年實體版面定位那輪（memory monster-entity-found）；程式目前沒讀它，
#   留著當版面文件 —— 搬家只影響文件正確性，不會壞功能。
OFF_PREV = 0x08           # 雙向串列
# ⚠ 出處同上 —— 雙向串列的 next；一樣沒被程式使用。
OFF_NEXT = 0x0C
OFF_POS_X = 0xBC          # 位置 X（見 TILE_UNITS）
OFF_POS_Y = 0xC0          # 位置 Y
OFF_ID = 0x1C8            # 實體 ID，每隻唯一 —— 攻擊目標就是傳這個
# ⚠ 結構偏移（大更新改版面才會壞）；出處＝檔頭那輪五台實測的版面定位。
OFF_TYPE = 0x1D0          # 種類 ID；0 = 其他玩家
# ⚠ 出處同上；名字內嵌 UTF-8、與玩家物件同版面（見檔頭 docstring），
#   讀取端以 NAME_MAX 截斷＋UTF-8 解不開就丟棄，垃圾值不會流出去。
OFF_NAME = 0x1D4
# ★ 實體種類。實測（同一畫面同時有三種）：
...
STATE_RUNNING = "Run"     # 跑動中
# ★★★ 「牠還在動」＝出手中或跑動中。2026-09-09 起這是死活判斷的一半：
#   血量百分比是 0~100 的**整數**，王剩不到 1% 就是 0，光看 0 會把還在打人的
#   王當屍體（使用者實機抓到）。會動的一律當活的。
BUSY_STATES = ATT_STATES + (STATE_RUNNING,)


def looks_dead(state: str, hp_pct: int) -> bool:
    """這隻死了沒 —— **掛機頁與副本頁共用這一份判斷**（使用者 2026-09-09：
    「同樣東西盡量不要有 2 個」）。

    兩個訊號，任一個成立就是死：
      · 動畫狀態 `'Dead'` —— 最快也最準（實測 64 隻死怪只有 1 隻後來變回 Wait）
      · 血量恰好 0 **而且沒在動作** —— 副本裡的柱子死了狀態不會變 'Dead'，
        只能靠血量；但血量是整數百分比，所以一定要加「沒在動作」這一半，
        不然剩不到 1% 的王會被當成屍體。

    ⚠ hp_pct = −1 是「沒交戰／讀不到」，一律當活的（讀不到 ≠ 沒有）。
    ⚠ 狀態讀不到（空字串）時只認血量那條 —— 那是靜止物件（柱子）的情形。
    """
    if state == STATE_DEAD:
        return True
    return hp_pct == 0 and state not in BUSY_STATES
...
class Entity:
    ...
    def hp_zero(self) -> bool:
        """血量歸零而且**沒在動作** ＝ 打死了（見 `looks_dead`）。

        ⚠⚠ 只有「恰好 0 而且靜止」才算：血量是 0~100 的**整數百分比**，
          王剩不到 1% 就讀成 0 —— 光看 0 會把還在打人的王當屍體
          （2026-09-09 使用者實機抓到）。−1 是沒交戰／讀不到，一律當活的。
        """
        return looks_dead(self.state, self.hp) and self.state != STATE_DEAD
...
def is_sitting(scanner, player_obj: int) -> bool:
    ...
    ⚠ **2026-08-09 起沒有任何呼叫端**：唯一用到它的「坐下休息」已依使用者
      要求移除（見 farm_tab 建構式裡那段）。留著是因為底下那些數字是實測過的
      遊戲知識、而且這支是零成本的純讀 —— 刪掉等於把知識一起丟了。
      要清死碼的話這支可以刪，但請先把實測數字搬進 memory。
