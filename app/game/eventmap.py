...
def enter(mover, scanner, route: EventMap, sub: int = 0,
    ...
    ⛔⛔ **不准在這裡自己再走一段**：走近／點擊／點不開換站位**全部交給
      `supply.talk_to_npc`**，跟藥水雜貨商人同一套（2026-09-06 使用者定：我們的地形圖
      走路站到離 NPC 最近的可走格 → 切官方 TryAct 講話 → 沒開往他身上擠再切官方；
      人牆＝走不動 APPROACH_STALL 秒就切官方，不磨逾時）。
      第一版在這裡先 _walk_to_npc 走到 NPC 那格、再交給 _engage 用遊戲尋路走一次 →
      兩套打架，天使學園廣場整天圍滿人只會磨到逾時（8/27 使用者實機回報「人太多會走不到」）。
    """
    def note(m):
        if say:
            say(m)
