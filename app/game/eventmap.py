...
「暴走穗海農場」（場景 441，2026-08-25 改版加的啤酒節活動）
  · 入口 NPC ＝ **啤酒節使者 14897**，站在**天使學園（41）(167, 91)**（9/22 版；原 170,90）。
...
ENTER_WAIT = 20.0
# 「對話開了、選項也送了，卻沒換到目標視窗」最多重試幾輪
# （交給 supply._engage_npc 自己數；走近＋補點本來就由它在 20 秒內自己處理）。
ENTER_TRIES = 5
...
ROUTES: tuple[EventMap, ...] = (
    EventMap(
        ...
        npc_scene=41,              # 天使學園
        npc_pos=(167, 91),         # 9/22 版 SP*.XML 擺在 (167,91)（原 170,90）；eventmap_check 對表
...
def enter(mover, scanner, route: EventMap, sub: int = 0,
    ...
    ⛔⛔ **不准自己先用地形圖走到 NPC 那一格**（2026-08-27 使用者實機回報：
      ...
      同一個坑：各步驟都不先自己用地形圖走過去。天使學園廣場整天圍滿人，
      那條路只會磨到逾時。
      → 走近／點擊／補點**全部交給 `supply.talk_to_npc`**，跟藥水雜貨商人
        同一套（官方尋路走過去、最後一段給 TryAct，每 2 秒補一發）。
    """
    def note(m):
        if say:
            say(m)
