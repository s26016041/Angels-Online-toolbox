...
# --- 自動回連 ---------------------------------------------------------------
# 帳號帳密都在這個分頁，重登也走這個分頁的一鍵登入流程，所以勾選放這裡
# （0.4.32 原本放在分身總控，0.4.33 搬過來）。
...
class LoginTab(BaseTab):
    ...
    def build_ui(self) -> None:
        ...
        # ⚠ 自動回連自己一顆固定節奏的計時器，跟上面那顆單發的無關 ——
        #   人不在電腦前才最需要它，分頁開不開著都要照跑。
    ...
    def _ar_toggled(self, on: bool) -> None:
        config.set("login.auto_reconnect", bool(on))
    ...
    def _ar_step(self) -> None:                       # noqa: C901
        ...
        if not self._ar_want:
            return
        err = self.start_auto_login(sorted(self._ar_want))
        if err:
