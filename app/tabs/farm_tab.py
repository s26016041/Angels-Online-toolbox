...
BALL_GAP = 8.0
# ★ 補貨失敗之後隔多久再試（**使用者 2026-08-26 定：10 秒**）。
...
# ⛔⛔ CORPSE_SECS（「鎖了這麼久還沒看到血量 ＝ 別人先打死的屍體」）**2026-09-13 刪除**。
#   使用者：「我們已經可以 100% 確認怪物死亡，還需要這個嗎」——不需要，而且它做的事
#   正好是**用軟訊號推翻硬訊號**：`looks_dead()` 已經說「牠還活著」，它再用「沒看到
#   血量」把牠當屍體丟掉。
#   · 真屍體根本輪不到它：挑目標第一關就是 `looks_dead()`（動畫 'Dead'／物件被遊戲
#     歸還／死亡旗標 +0x3D6==7），三個訊號實測不會漏（一般怪當拍就 'Dead'，64 隻只有
#     1 隻回退；活怪 7,800 次取樣沒出現過旗標 7）。
#   · 它抓到的幾乎全是**血量填得慢的活怪**：2026-09-13 五台實測 242 個樣本，
#     「寫進目標欄 → 遊戲填出血量」中位 0.10~0.20 秒，但 **9~17% 超過 0.8 秒、最久
#     3.23 秒** → 約每 10 隻活怪冤枉 1 隻，還冰 NOHP_MEMORY(20 秒)。周圍常常只有
#     1~2 隻怪，於是就是使用者回報的「周圍明明有怪卻完全不打，最後被別人搶走」。
#   · 「鎖了卻交戰不上」那種（選定送不出去…）**不是死活問題**，STUCK_ENGAGED(15 秒)
#     本來就在收尾，冷卻也是同一個 NOHP_MEMORY。
#   ⛔ 不准再用「沒看到血量」推死活（跟「血量歸零＝死」同一類的猜測，見 looks_dead）。
...
class TargetWorker(_Paced):
    ...
    def __init__(self, sc: MemoryScanner) -> None:
        ...
        self._wrote = False
        # 這隻有沒有讀到過 > 0 的血量 —— **只拿來分辨「這算不算我的擊殺」**
        # （屍體 vs 交戰過），⛔ 不判死（見 CORPSE_SECS 那段刪除紀錄）。
        self._saw_hp = False
    ...
    def step(self) -> None:
        ...
        try:
            ...
            #   50Hz 就讀得到（唯一的死亡訊號，見 CORPSE_SECS 那段刪除紀錄）。
            ...
            # ⛔ 血量歸零**不是死亡訊號**（2026-09-09 使用者實機抓到）：那是 0~100 的
            #   整數百分比，王剩不到 1% 就是 0。死活一律問 `entity.looks_dead()`。
            # ⛔⛔ 「遊戲把目標欄清 0（cur == 0）」也**不是**死亡訊號（2026-09-09
            #   使用者定：「拿掉」）：換地圖、目標失效、使用者自己點別的目標都會清
            #   → 全部會被誤記成一次擊殺。⚠ `cur` 還是要讀 —— 底下寫目標欄前要先
            #   確認這個位址還是狀態物件。
            # ⛔⛔ 「鎖了這麼久還沒看到血量 ＝ 屍體」也在 2026-09-13 拿掉了
            #   （冤枉 11% 的活怪，理由見 CORPSE_SECS 那段刪除紀錄）。
            #   → 這一支現在**只認 `looks_dead()`**，別的都不算死。
...
class CharFarmPage(QWidget):
    ...
    def __init__(self, pid: int, hwnd: int, title: str, sc: MemoryScanner,
        ...
        # ⚠ 補貨失敗之後隔多久才再試。**不是永久放棄** —— 舊版一失敗就閂到
        #   球不滿為止，於是「滿了卻永遠不動」而且畫面上沒有任何說明
        #   （2026-08-22 使用者回報）。被拒的購買不扣點，隔一段時間再試是安全的
        #   （而且 balls.restock 會先看商城倉庫有沒有現成的，不會重複買）。
    ...
    def _ball_done(self, ok: bool, msg: str) -> None:
        ...
        if "定位失敗" in msg:
            ...
            #   這種不會自己好：排 10 分鐘冷卻只是每 10 分鐘再白跑一輪
            #   （三次購買重試、各等滿 6 秒節流），而且只通知第一次 ——
            #   使用者 2026-08-23 回報的「卡在那邊」就是這個。
            #   → 停用＋講清楚怎麼恢復（儲值後把勾拿掉再勾一次）。
            self._ball_off = msg
            self.notify(f"自動換球已停用：{msg}　"
                        f"（儲值後把「自動換球」的勾拿掉再勾一次就會重試）")
        else:
            # ★ 失敗**不是永久放棄**：排一次冷卻之後再試（被拒的購買不扣點，
            #   而且 balls.restock 會先看商城倉庫有沒有現成的）。
            self._ball_retry_at = time.monotonic() + BALL_RETRY_GAP
            self._ball_lbl.setText(f"經驗球：⚠ {msg}")
            if not self._ball_told:      # 同一個「都滿了」事件只吵一次
    ...
    def _ball_status(self, sc) -> tuple[str, list | None, object]:
        ...
        ★★ **每一個「這輪不動作」的出口都要說得出理由**（2026-08-22 使用者
           回報：「滿了沒買新的換上，看不出來問題在哪」）。舊版有十個安靜的
           `return`，畫面上一個字都沒有 —— 功能沒動跟功能沒開長得一模一樣。
        ⚠ 這支**純讀、不做任何動作**，所以可以每一拍叫（也給畫面標籤用）。
        回傳的第二／第三個值是 None 代表「這輪不能動手」。
        """
        ...
        # 上限讀不到就無從判斷滿沒滿 —— 這是最容易「安靜什麼都不做」的一種。
        blind = [b for b in cur if not b.known]
        if blind:
            return (f"⚠ 讀不到上限（{blind[0].name}）→ 不判斷滿沒滿",
                    None, None)
        ...
        if missing:
            # ★★ 順序很重要：**便宜的純讀預檢排在冷卻前面**。
            #   `mall.blocked()` 只是讀幾個值，說得出「為什麼現在不可能成功」。
            #   擺在冷卻後面的話，就算造成失敗的原因早就解除（例如你清出了
            #   背包空格），也要把冷卻等完才會發現 —— 使用者 2026-08-26 問的
            #   「不能馬上重試嗎」就是這個。現在原因一解除，下一拍就會動手。
            ...
            # ★ 過了預檢又過了冷卻才真的動手 —— 一輪補貨要跑三十秒
            #   （3 次購買重試 × 各等滿 6 秒節流），所以不能白跑
    ...
    def _candidates(self, quiet: bool = False) -> tuple[
        ...
        for m in self.mons:
            ...
            #     （最久 79.8 秒）。挑最近的就常常挑到牠們 —— 所以這一關一定要
            #     當場重讀死活，不能只信掃描那一拍的結果。
    ...
    def _switch_closer(self, cur, dist: float | None) -> bool:
        ...
        if self._hurt or dist is None or now < self._switch_t:
            return False
        # ★ 首發已經確認打在牠身上（施放廣播）＝打過了，不准換 —— 血量那一拍還沒讀到掉血
        #   （_hurt 還是 False）的空窗裡換走，就是「放完首發跑去打別隻、又放一次首發」
        #   （2026-09-21 使用者回報「變成一直用首發攻擊」）。
        k = self._keys
        if k.opener_vk and k._opened and k._open_eid == cur.eid:
            return False
