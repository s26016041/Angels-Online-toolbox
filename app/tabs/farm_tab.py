...
class CharFarmPage(QWidget):
    ...
    def tick(self, dt: float) -> None:
        ...
        # ★★ 施放封包的座標**跟遊戲一樣送 0**（使用者指出的）。
        #   攔包實證（雪狐、黑狐各一份）：遊戲自己送的是
        #       0x664627(技能ID, 目標ID, 0, 0, 0)
        #   我們卻一直帶著怪的格子座標，是我們跟遊戲之間最後一處沒對齊的地方。
        # ⚠ 代價：**順移那類對地技能會發不動**（它要靠座標指定落點，
        #   見 [[teleport-skill]]）。把順移放在攻擊鍵的話要改回帶座標。
        self._keys.pos = (0, 0)
