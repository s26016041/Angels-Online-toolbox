...
class FakeGrid:
    ...
    # ★ 真的那支是問「把場景物件的阻擋拿掉，會不會就走得到」
    #   （terrain.Grid.gate_between）。測試裡用一個旗標直接指定答案。
    gate = False
    gate_calls = 0

    def gate_between(self, here, goal):
        # ⚠ 真的那支要泛洪兩次（實機 440x280 量到 41.6ms）——測試要盯著呼叫次數
        type(self).gate_calls = getattr(self, "gate_calls", 0) + 1
        self.gate_calls = type(self).gate_calls
...
def main() -> int:
    ...
    dt.team.deny = lambda mv: (world["denied"].append(id(mv)), True)[1]
    ...
    # ②c 隊長自己接到別人的邀請（名單裡是別人）→ 馬上重走，不等這一輪過完
    tab = loop_tab("bind")
    tab._phase = "enter"
    world.update(mine=[], his=[], left=[], invited=[], joined=0, denied=[], here=90)
    tab._team_begin()
    ...
    world.update(mine=[M("路人")], his=[], left=[], invited=[], joined=0, denied=[],
                 here=90)
    tab._phase = "enter"
    tab._team_begin()
    run(tab, 0.3)
    ...
    world["mine"] = []
    run(tab, 0.3)
    ck("　清空後改成等遊戲配隊（不邀請任何人、也不送拒絕）", tab._team_sub == "wait"
       and not world["invited"] and not world["denied"],
       f"{tab._team_sub} {world['invited']} denied={world['denied']}")
    run(tab, 1.0)
    ...
    # =====================================================================
    # ★★★ 機關擋路：走不到，但「拿掉場景物件的阻擋就走得到」→ 回頭再撞一次
    #      2026-09-07 黑狐 莉薇坦的寢室，使用者：「這種東西就是多撞幾次就會開」
    # =====================================================================
    print("")
    print("機關擋路 → 回頭再撞")
    steps = [{"do": dungeon.INTERACT, "at": [10.0, 10.0], "model": 60413, "menu": []},
             {"do": dungeon.WALK, "to": [90, 90]}]
    tab = make_tab(steps, pos=(10.0, 10.0))
    tab._grid = FakeGrid([(10, 10)], others=[(90, 90)])
    tab._grid.gate = True
    tab._i = 1
    tab._blocked(TICK, "走不到 (90, 90)", (90, 90))
    ck("★ 第一拍先等（不會每拍都跳回去）", tab._i == 1, f"i={tab._i}")
    tab._blocked(dt.GATE_RETRY + 0.1, "走不到 (90, 90)", (90, 90))
    ck("★ 隔了間隔 → 跳回前一個「點物件」的步驟", tab._i == 0, f"i={tab._i}")
    ck("　撞的次數有留著（_goto 會歸零，要補回去）", tab._gate_n == 1, str(tab._gate_n))
    ck("　沒有因此停機", tab.run_cb.isChecked())
    # 機關已經開了（gate_between 回 False）→ 照舊只回報，不跳回去
    tab = make_tab(steps, pos=(10.0, 10.0))
    tab._grid = FakeGrid([(10, 10)], others=[(90, 90)])
    tab._grid.gate = False
    tab._i = 1
    tab._blocked(dt.GATE_RETRY + 1.0, "走不到 (90, 90)", (90, 90))
    ck("★ 不是機關擋的（地形本來就不通）→ 不跳回去", tab._i == 1, f"i={tab._i}")
    # 前面沒有互動步驟 → 只回報，不跳、不當掉
    tab = make_tab([{"do": dungeon.WALK, "to": [90, 90]}], pos=(10.0, 10.0))
    tab._grid = FakeGrid([(10, 10)], others=[(90, 90)])
    tab._grid.gate = True
    tab._i = 0
    tab._blocked(dt.GATE_RETRY + 1.0, "走不到 (90, 90)", (90, 90))
    ck("★ 腳本裡沒有「點物件」的步驟 → 只回報、不跳、不停機",
       tab._i == 0 and tab.run_cb.isChecked(), f"i={tab._i}")
    # 地形圖答不出來（舊的替身沒有 gate_between）→ 安全退化
    tab = make_tab(steps, pos=(10.0, 10.0))

    class _OldGrid(FakeGrid):
        gate_between = None

    tab._grid = _OldGrid([(10, 10)], others=[(90, 90)])
    tab._i = 1
    tab._blocked(dt.GATE_RETRY + 1.0, "走不到 (90, 90)", (90, 90))
    ck("★ 地形圖答不出來 → 安全退化（不跳回去、不當掉）", tab._i == 1, f"i={tab._i}")
    # ⚠⚠ 成本：gate_between 要泛洪兩次（實機 41.6ms），而 _blocked 是走不到時
    #   **每一拍**都會進來的 —— 每拍重算會讓整個工具箱變頓（no-crash-no-lag）。
    tab = make_tab(steps, pos=(10.0, 10.0))
    grid = FakeGrid([(10, 10)], others=[(90, 90)])
    grid.gate = False
    tab._grid = grid
    tab._i = 1
    FakeGrid.gate_calls = 0
    for _ in range(40):
        tab._blocked(TICK, "走不到 (90, 90)", (90, 90))
    ck("★★ 冷卻中不重算：40 拍只問地形圖幾次（不是 40 次）",
       FakeGrid.gate_calls <= int(40 * TICK / dt.GATE_RETRY) + 1,
       f"問了 {FakeGrid.gate_calls} 次")

    # ---- terrain：bit0（場景物件放的）跟 bit1（地圖本身的牆）要分得開 ----
    print("")
    print("地形旗標：物件阻擋 vs 地圖的牆")
    from app.game import terrain as _terr
    W = H = 5

    def _mk(fl):
        return _terr.Grid(
            W, H, 0,
            [bytearray(0 if (f & _terr.BLOCK_MASK) else 1 for f in row) for row in fl],
            [bytearray(0 if (f & _terr.MAP_MASK) else 1 for f in row) for row in fl])

    # x=2 整行不可走：上半是「物件放的」(bit0)、下半是地圖的牆 (bit1)
    flags = [[0] * W for _ in range(H)]
    for y in range(H):
        flags[y][2] = _terr.OBJ_MASK if y < 3 else _terr.MAP_MASK
    g = _mk(flags)
    ck("walkable：兩種 bit 都擋（跟遊戲的 test …,3 一樣）",
       not g.walkable(2, 0) and not g.walkable(2, 4))
    ck("map_walkable：只有地圖的牆才擋",
       g.map_walkable(2, 0) and not g.map_walkable(2, 4))
    ck("object_blocked：分得出「只被物件擋住」",
       g.object_blocked(2, 0) and not g.object_blocked(2, 4))
    ck("★ gate_between：左右現在走不通，但拿掉物件阻擋就通 → True",
       g.reachable(0, 0) is not None and (4, 0) not in g.reachable(0, 0)
       and g.gate_between((0, 0), (4, 0)))
    flags2 = [[0] * W for _ in range(H)]
    for y in range(H):
        flags2[y][2] = _terr.MAP_MASK
    ck("★ 整片是地圖的牆 → gate_between 回 False（不是機關）",
       not _mk(flags2).gate_between((0, 0), (4, 0)))
    ck("★ 本來就走得到 → gate_between 回 False", not g.gate_between((0, 0), (1, 0)))
    ck("★ 目標那格自己被物件蓋住 → 回 False（那是腳本點位壓到擺設）",
       not g.gate_between((0, 0), (2, 0)))
    ck("　沒有 soft（舊的 Grid）→ 安全退化回 False",
